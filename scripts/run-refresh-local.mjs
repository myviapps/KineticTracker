...
if (!CRON) {
  console.error("Missing CRON_SECRET in .env (see .env.example)");
  process.exit(1);
}
...
(async () => {
  ...
  if (has("resume")) {
    ...
    if (!active.length) {
      console.error(C.red("No active job to resume."));
      process.exit(1);
    }
    jobId = active[0].id;
    ...
    if (scope === "classroom" && !classroomId) {
      const cls = (
        await (await fetch(`${U}/rest/v1/classrooms?select=id,name&limit=1`, { headers: H })).json()
      )[0];
      if (!cls) {
        console.error(C.red("No classrooms found."));
        process.exit(1);
      }
      classroomId = cls.id;
    ...
    jobId = await (
      await fetch(`${U}/rest/v1/rpc/enqueue_refresh_job`, {
        method: "POST",
        headers: H,
        body: JSON.stringify({
          p_scope: scope,
  ...
  while (Date.now() < deadline && chunk < MAX_CHUNKS) {
    ...
    try {
      ...
      if (r.status === 401) {
        console.error(
          C.red(
            "  401 — CRON_SECRET mismatch between .env and the running dev server. Restart `npm run dev`.",
          ),
        );
        process.exit(1);
      }
      ...
      idle++;
      if (idle > 15) {
        console.log(C.red("  giving up after 15 failures"));
        break;
      }
      await sleep(4000);
      continue;
    ...
    if (res.claimed === false) {
      ...
      idle++;
      if (idle > 20) {
        console.log(C.red("  lease never released — stopping"));
        break;
      }
    ...
    idle = 0;
    chunk++;
    const gained = res.processed - last;
    last = res.processed;
    ...
    if (res.done) {
      console.log(
        C.grn(
          `\n✔ COMPLETE in ${dur(Date.now() - t0)} — ${res.succeeded} updated, ${res.failed} failed`,
        ),
      );
      break;
    }
    if (res.paused) {
      console.log(C.yel("\n⏸ paused — rate limited, will resume automatically"));
      break;
    }
  }
  ...
  const j = (
    await (
      await fetch(
        `${U}/rest/v1/refresh_jobs?select=status,processed,succeeded,failed,total&id=eq.${jobId}`,
        { headers: H },
      )
    ).json()
  )[0];
  console.log(
    C.bold("\nfinal: ") +
      `${j.status}  ${j.processed}/${j.total}  ok=${j.succeeded} fail=${j.failed}`,
  );
  const rate = j.processed / ((Date.now() - t0) / 60000);
  if (j.processed)
    console.log(
      C.dim(
        `rate: ~${Math.round(rate)} students/min → 1000 students ≈ ${dur((1000 / rate) * 60000)}`,
      ),
    );
  console.log("");
})();
