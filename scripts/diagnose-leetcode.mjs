...
async function fetchStudentTimed(student) {
  ...
  const solved = p.ok
    ? p.data?.matchedUser?.submitStatsAll?.acSubmissionNum?.find((x) => x.difficulty === "All")
        ?.count
    : null;
...
async function timeWrite(studentId, r) {
  ...
  await step("daily_snapshots prev select", () =>
    fetch(
      `${SUPABASE_URL}/rest/v1/daily_snapshots?select=total_solved&student_id=eq.${studentId}&order=snapshot_date.desc&limit=1`,
      { headers: SB },
    ),
  );
...
(async () => {
  ...
  console.log(
    C.dim(
      `batch=${BATCH_SIZE}  cooldown=${COOLDOWN_MS}ms  batches=${BATCHES}  write=${DO_WRITE}\n`,
    ),
  );
  ...
  const probe = await gqlTimed("probe", students[0].leetcode_id, RECENT_QUERY, {
    username: students[0].leetcode_id,
    limit: 1,
  });
  if (probe.ok) {
  ...
  for (let b = 0; b < BATCHES; b++) {
    ...
    for (const r of results) {
      perStudent.push(r);
      const detail = r.calls
        .map(
          (c) => `${c.label}:${c.ok ? C.grn(ms(c.elapsed)) : C.red(`${c.kind}/${ms(c.elapsed)}`)}`,
        )
        .join("  ");
      const head =
        r.failed.length === 0 && !r.notFound
          ? C.grn("OK  ")
          : r.notFound
            ? C.yel("404 ")
            : C.red("FAIL");
      log(
        `  ${head} ${String(r.username).padEnd(22)} ${String(ms(r.total)).padStart(7)}   ${detail}`,
      );
      ...
      if (r.notFound)
        log(
          C.yel(
            `         └─ leetcode_id "${r.username}" does not exist (student: ${r.student.name} / ${r.student.roll})`,
          ),
        );
    }
    ...
    if (DO_WRITE) {
      ...
      log(
        C.cyn(`  writes: ${ms(bWrite)} for ${writes.length} students`) +
          (slowest ? C.dim(`  (slowest step: ${slowest.name} ${ms(slowest.elapsed)})`) : ""),
      );
      for (const w of writes)
        for (const s of w.steps)
          if (!s.ok) log(C.red(`         └─ write FAIL ${s.name}: ${s.note}`));
    }
  ...
  const avgStudent = Math.round(
    perStudent.reduce((s, r) => s + r.total, 0) / Math.max(1, perStudent.length),
  );
  const avgBatch = Math.round(
    perBatch.reduce((s, b) => s + b.total, 0) / Math.max(1, perBatch.length),
  );
  ...
  console.log(
    `  succeeded          : ${okCount === perStudent.length ? C.grn(okCount) : C.yel(okCount)}`,
  );
  console.log(`  bad leetcode_id    : ${nf ? C.yel(nf) : 0}`);
  console.log(`  throttled calls    : ${throttled ? C.red(throttled) : C.grn(0)}`);
  console.log(
    `  avg per student    : ${ms(avgStudent)}  ${C.dim("(3 serial calls + 500ms of built-in sleeps)")}`,
  );
  console.log(`  avg batch wall     : ${ms(avgBatch)} for ${BATCH_SIZE} concurrent`);
  console.log(
    `  batch period       : ${ms(period)} ${C.dim(`(wall + ${COOLDOWN_MS}ms cooldown)`)}`,
  );
  console.log(
    C.bold(`  throughput         : ~${perMin} students/min   (~${reqPerMin} req/min at LeetCode)`),
  );
  ...
  if (avgBatch > chunkBudget) {
    console.log(
      C.red(
        `  ⚠ One batch (${ms(avgBatch)}) exceeds the whole chunk budget — the worker can never commit.`,
      ),
    );
  }
  console.log("");
